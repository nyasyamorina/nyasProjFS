...
public static partial class FileSystemApi
{
    [Flags]
    ...
    [LibraryImport("kernel32.dll", SetLastError = true, StringMarshalling = StringMarshalling.Utf16)]
    private static partial int CreateSymbolicLinkW(string lpSymlinkFileName, string lpTargetFileName, SymbolicLinkOptions dwFlags);

    [LibraryImport("shlwapi.dll", SetLastError = true, StringMarshalling = StringMarshalling.Utf16)]
    [return: MarshalAs(UnmanagedType.Bool)]
    private static partial bool PathRelativePathToW(
        [Out] ushort[] pszPath,
        string pszFrom,
    ...
    public static string TryGetPathRelativeToRoot(string root, string path, bool isPathToADirectory)
    {
        ...
        ushort[] relativePathBuilder = new ushort[MaxPath];
        ...
        unsafe {
            fixed (ushort* buffer = relativePathBuilder) return new string((char*) buffer);
        }
    }
}